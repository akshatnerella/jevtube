...
This repo is the **client only**. Classification runs on the shared [jev-backend](https://github.com/akshatnerella/jev-backend) at `POST https://jev-backend.vercel.app/api/jevtube/classify`, which holds the API keys, caching and rate limits for every Jev app.
...
scripts/     package.sh builds dist/jevtube-<version>.zip for upload.
```

```
content.js (YouTube tab)                        background.js                 jev-backend
  finds tiles near the viewport,        ──▶     local cache,          ──▶    /api/jevtube/classify
...
**Default categories** live in `extension/settings.js`. The backend's copy (`apps/jevtube.js` in jev-backend) is only used by clients that send no categories.
